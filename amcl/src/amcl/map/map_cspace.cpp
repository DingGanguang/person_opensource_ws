...
{
public:
	CachedDistanceMap(double scale, double max_dist) : // 入参：分辨率、最大距离
													   distances_(NULL), scale_(scale), max_dist_(max_dist)
	{
		cell_radius_ = max_dist / scale; // cell_radius_ 就是地图的宽度；
		...
		for (int i = 0; i <= cell_radius_ + 1; i++)
		{
			...
			{
				distances_[i][j] = sqrt(i * i + j * j); // 创建了一个二维数组，大小为：（cell_radius+2）*(cell_radius+2)
			}
		}
	}
	~CachedDistanceMap()
	...
	double **distances_; // distances_ 二维指针数组
	double scale_;
	double max_dist_;
	int cell_radius_;
};

bool operator<(const CellData &a, const CellData &b) // CellData地图对象：  比较两个地图的指定栅格中的最大占据栅格的距离比较
{
