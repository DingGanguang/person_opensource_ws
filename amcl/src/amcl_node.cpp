...
void AmclNode::updatePoseFromServer() // 这里是初始化位姿的前身，还处于设置均值和协方差的阶段：   
...

{
    ...
    map_ = convertMap(msg);     // AMCLCMT: 创建地图对象amcl自己的地图格式
    ...
    // Initialize the filter  // AMCLCMT: 构建粒子滤波器对象，并完成初始化。
    updatePoseFromServer();
    pf_vector_t pf_init_pose_mean = pf_vector_zero();
